...
public class NoSeatTicket  implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		JButton sureButton = new JButton("无座购票");sureButton.setFont(new Font("黑体",Font.PLAIN,20));//设置字体的字体，大小
		...
		ActionListener listener1 = new ActionListener()    //购票监听
				{
					...
					{   String shu = f1.getText();
						...
						try {
				            ...
				            String  s1="UPDATE TICKET_LEVEL SET 无座票数 =无座票数-'"+shu+"'  WHERE 列车号 = '"+piao+"'";
					...
					{     
						...
						ResultSet rs=stmt.executeQuery(s);   //游标 查询表内内容
						if(!rs.next()) {
							String  s2= "insert into TICKER_BUYER(用户名,列车号,起始站,终点站,无座票数) values('"+LoginUI.a+"','"+piao+"','"+c+"','"+c1+"','"+shu+"')";
							...
							String  s2="UPDATE TICKER_BUYER  SET 无座票数 = 无座票数+'"+shu+"' WHERE 列车号 = '"+piao+"'and 用户名 ='"+LoginUI.a+"'and 起始站 ='"+c+"'and 终点站 ='"+c1+"'";
     ...
     public NoSeatTicket(String city,String city1){
    	 c=city;c1=city1;
     }
}
