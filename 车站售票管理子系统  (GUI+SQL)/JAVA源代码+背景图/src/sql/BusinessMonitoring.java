...
public class BusinessMonitoring  implements ActionListener {
	String c;
	String c1;
	JComboBox<String>comboBox = new JComboBox<>();
	public void actionPerformed(ActionEvent e) {
   		JFrame f = new JFrame("购买车票");
		JButton sureButton = new JButton("商务购票");sureButton.setFont(new Font("黑体",Font.PLAIN,20));//设置字体的字体，大小
		...
		ActionListener listener1 = new ActionListener()    //购票监听
				{
					...
					{   String shu = f1.getText();
						...
						try {
				            ...
				            String  s1="UPDATE TICKET_LEVEL SET 商务票数 =商务票数-'"+shu+"'  WHERE 列车号 = '"+piao+"'";
					...
					{     
						...
						ResultSet rs=stmt.executeQuery(s);   //游标 查询表内内容
						if(!rs.next()) {
							String  s2= "insert into TICKER_BUYER(用户名,列车号,起始站,终点站,商务票数) values('"+LoginUI.a+"','"+piao+"','"+c+"','"+c1+"','"+shu+"')";
							int r2=stmt1.executeUpdate(s2);
							if(r2==1) {
								JOptionPane.showMessageDialog(null, "购票成功","提示消息",JOptionPane.PLAIN_MESSAGE);
							}
						}else {
							String  s2="UPDATE TICKER_BUYER  SET 商务票数 = 商务票数+'"+shu+"' WHERE 列车号 = '"+piao+"'and 用户名 ='"+LoginUI.a+"'and 起始站 ='"+c+"'and 终点站 ='"+c1+"'";
     ...
     public BusinessMonitoring(String city,String city1){
    	 c=city;c1=city1;
     }
}
